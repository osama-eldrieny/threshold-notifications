...
const isPlainObject = require('../../../../is-plain-object@5.0.0/node_modules/is-plain-object/dist/is-plain-object.cjs');
...
var resolveDescendants = children => {
  ...
  var addChild = child => {
    ...
    if (index_es.Text.isText(child)) {
      var c = child; // HACK: fix typescript complaining
      if (index_es.Text.isText(prev) && STRINGS.has(prev) && STRINGS.has(c) && index_es.Text.equals(prev, c, {
        loose: true
      })) {
        prev.text += c.text;
      } else {
    ...
    } else if (index_es.Element.isElement(child)) {
      nodes.push(child);
    } else if (child instanceof Token) {
      var n = nodes[nodes.length - 1];
      if (!index_es.Text.isText(n)) {
        addChild('');
...
var createEditor = makeEditor => (tagName, attributes, children) => {
  ...
  for (var child of children) {
    if (index_es.Range.isRange(child)) {
  ...
  for (var [node, path] of index_es.Node.texts(editor)) {
  ...
  } else if (index_es.Range.isRange(selection)) {
...
var DEFAULT_CREATORS = {
  anchor: createAnchor,
  cursor: createCursor,
  editor: createEditor(index_es.createEditor),
