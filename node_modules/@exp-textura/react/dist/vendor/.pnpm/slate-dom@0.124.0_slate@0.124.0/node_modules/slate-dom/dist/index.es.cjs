...
var DOMEditor = {
  ...
  deselect: editor => {
    ...
    if (selection) {
      index_es.Transforms.deselect(editor);
    }
  },
  ...
  findEventRange: (editor, event) => {
    ...
    if (index_es.Node.isElement(node) && index_es.Editor.isVoid(editor, node)) {
      var rect = target.getBoundingClientRect();
      var isPrev = editor.isInline(node) ? x - rect.left < rect.left + rect.width - x : y - rect.top < rect.top + rect.height - y;
      var edge = index_es.Editor.point(editor, path, {
        edge: isPrev ? 'start' : 'end'
      });
      var point = isPrev ? index_es.Editor.before(editor, edge) : index_es.Editor.after(editor, edge);
      if (point) {
        var _range = index_es.Editor.range(editor, point);
        return _range;
      }
    }
  ...
  findPath: (editor, node) => {
    ...
    throw new Error("Unable to find the path for Slate node: ".concat(index_es.Scrubber.stringify(node)));
  },
  focus: function focus(editor) {
    ...
    if (root.activeElement !== el) {
      ...
      if (!editor.selection) {
        index_es.Transforms.select(editor, index_es.Editor.start(editor, []));
      }
  ...
  hasRange: (editor, range) => {
    ...
    return index_es.Editor.hasPath(editor, anchor.path) && index_es.Editor.hasPath(editor, focus.path);
  },
  ...
  isTargetInsideNonReadonlyVoid: (editor, target) => {
    ...
    return index_es.Node.isElement(slateNode) && index_es.Editor.isVoid(editor, slateNode);
  },
  ...
  toDOMNode: (editor, node) => {
    ...
    if (!domNode) {
      throw new Error("Cannot resolve a DOM node from Slate node: ".concat(index_es.Scrubber.stringify(node)));
    }
    return domNode;
  },
  toDOMPoint: (editor, point) => {
    var [node] = index_es.Editor.node(editor, point.path);
    var el = DOMEditor.toDOMNode(editor, node);
    var domPoint;
    // If we're inside a void node, force the offset to 0, otherwise the zero
    // width spacing character will result in an incorrect offset of 1
    if (index_es.Editor.void(editor, {
      at: point
    })) {
      point = {
        path: point.path,
    ...
    if (!domPoint) {
      throw new Error("Cannot resolve a DOM point from Slate point: ".concat(index_es.Scrubber.stringify(point)));
    }
    return domPoint;
  },
  toDOMRange: (editor, range) => {
    ...
    var isBackward = index_es.Range.isBackward(range);
    var domAnchor = DOMEditor.toDOMPoint(editor, anchor);
    var domFocus = index_es.Range.isCollapsed(range) ? domAnchor : DOMEditor.toDOMPoint(editor, focus);
    ...
    if (IS_ANDROID && !textNode && !exactMatch) {
      ...
      if (node && DOMEditor.hasDOMNode(editor, node, {
        ...
        var {
          path: _path,
          offset: _offset
        } = index_es.Editor.start(editor, nodePath);
  ...
  toSlateRange: (editor, domRange, options) => {
    ...
    if (index_es.Range.isExpanded(range) && index_es.Range.isForward(range) && isDOMElement(focusNode) && index_es.Editor.void(editor, {
      at: range.focus,
      mode: 'highest'
    })) {
      range = index_es.Editor.unhangRange(editor, range, {
        voids: true
      });
    }
    return range;
...
function verifyDiffState(editor, textDiff) {
  ...
  if (!index_es.Editor.hasPath(editor, path)) {
    return false;
  }
  var node = index_es.Node.get(editor, path);
  if (!index_es.Node.isText(node)) {
    return false;
  }
  if (diff.start !== node.text.length || diff.text.length === 0) {
    return node.text.slice(diff.start, diff.start + diff.text.length) === diff.text;
  }
  var nextPath = index_es.Path.next(path);
  if (!index_es.Editor.hasPath(editor, nextPath)) {
    return false;
  }
  var nextNode = index_es.Node.get(editor, nextPath);
  return index_es.Node.isText(nextNode) && nextNode.text.startsWith(diff.text);
}
...
function normalizePoint(editor, point) {
  ...
  var leaf = index_es.Node.get(editor, path);
  if (!index_es.Node.isText(leaf)) {
    return null;
  }
  var parentBlock = index_es.Editor.above(editor, {
    match: n => index_es.Node.isElement(n) && index_es.Editor.isBlock(editor, n),
    at: path
  });
  if (!parentBlock) {
    return null;
  ...
  while (offset > leaf.text.length) {
    var entry = index_es.Editor.next(editor, {
      at: path,
      match: index_es.Node.isText
    });
    if (!entry || !index_es.Path.isDescendant(entry[1], parentBlock[1])) {
      return null;
    }
...
function normalizeRange(editor, range) {
  ...
  if (index_es.Range.isCollapsed(range)) {
    return {
      anchor,
      focus: anchor
    };
...
function transformPendingPoint(editor, point, op) {
  ...
  var textDiff = pendingDiffs === null || pendingDiffs === void 0 ? void 0 : pendingDiffs.find(_ref => {
    ...
    return index_es.Path.equals(path, point.path);
  });
  if (!textDiff || point.offset <= textDiff.diff.start) {
    return index_es.Point.transform(point, op, {
      affinity: 'backward'
    });
  }
  var {
  ...
  if (point.offset <= diff.start + diff.text.length) {
    ...
    var _transformed = index_es.Point.transform(_anchor, op, {
      affinity: 'backward'
    });
    if (!_transformed) {
      return null;
  ...
  var transformed = index_es.Point.transform(anchor, op, {
    affinity: 'backward'
  });
  if (!transformed) {
    return null;
  }
  if (op.type === 'split_node' && index_es.Path.equals(op.path, point.path) && anchor.offset < op.position && diff.start < op.position) {
    return transformed;
  }
  return {
...
function transformTextDiff(textDiff, op) {
  ...
  switch (op.type) {
    case 'insert_text':
      {
        if (!index_es.Path.equals(op.path, path) || op.offset >= diff.end) {
          return textDiff;
        }
    ...
    case 'split_node':
      {
        if (!index_es.Path.equals(op.path, path) || op.position >= diff.end) {
          return {
            diff,
            id,
            path: index_es.Path.transform(path, op, {
              affinity: 'backward'
            })
          };
        }
    ...
    case 'merge_node':
      {
        if (!index_es.Path.equals(op.path, path)) {
          return {
            diff,
            id,
            path: index_es.Path.transform(path, op)
  ...
  var newPath = index_es.Path.transform(path, op);
  if (!newPath) {
    return null;
  }
  return {
...
var findCurrentLineRange = (editor, parentRange) => {
  var parentRangeBoundary = index_es.Editor.range(editor, index_es.Range.end(parentRange));
  var positions = Array.from(index_es.Editor.positions(editor, {
    at: parentRange
  }));
  var left = 0;
  var right = positions.length;
  var middle = Math.floor(right / 2);
  if (areRangesSameLine(editor, index_es.Editor.range(editor, positions[left]), parentRangeBoundary)) {
    return index_es.Editor.range(editor, positions[left], parentRangeBoundary);
  }
  if (positions.length < 2) {
    return index_es.Editor.range(editor, positions[positions.length - 1], parentRangeBoundary);
  }
  while (middle !== positions.length && middle !== left) {
    if (areRangesSameLine(editor, index_es.Editor.range(editor, positions[middle]), parentRangeBoundary)) {
      right = middle;
    } else {
      left = middle;
    }
...
var withDOM = function withDOM(editor) {
  ...
  e.deleteBackward = unit => {
    ...
    if (e.selection && index_es.Range.isCollapsed(e.selection)) {
      var parentBlockEntry = index_es.Editor.above(e, {
        match: n => index_es.Node.isElement(n) && index_es.Editor.isBlock(e, n),
        at: e.selection
      });
      if (parentBlockEntry) {
        var [, parentBlockPath] = parentBlockEntry;
        var parentElementRange = index_es.Editor.range(e, parentBlockPath, e.selection.anchor);
        var currentLineRange = findCurrentLineRange(e, parentElementRange);
        if (!index_es.Range.isCollapsed(currentLineRange)) {
          index_es.Transforms.delete(e, {
            at: currentLineRange
          });
        }
      }
  ...
  e.apply = op => {
    ...
    if (pendingAction !== null && pendingAction !== void 0 && pendingAction.at) {
      var at = index_es.Location.isPoint(pendingAction === null || pendingAction === void 0 ? void 0 : pendingAction.at) ? transformPendingPoint(e, pendingAction.at, op) : transformPendingRange(e, pendingAction.at, op);
    ...
    switch (op.type) {
      ...
      case 'remove_node':
        {
          matches.push(...getMatches(e, index_es.Path.parent(op.path)));
          break;
        }
      case 'merge_node':
        {
          var prevPath = index_es.Path.previous(op.path);
          matches.push(...getMatches(e, prevPath));
          break;
        }
      case 'move_node':
        {
          var commonPath = index_es.Path.common(index_es.Path.parent(op.path), index_es.Path.parent(op.newPath));
          matches.push(...getMatches(e, commonPath));
          var changedPath;
          if (index_es.Path.isBefore(op.path, op.newPath)) {
            matches.push(...getMatches(e, index_es.Path.parent(op.path)));
            changedPath = op.newPath;
          } else {
            matches.push(...getMatches(e, index_es.Path.parent(op.newPath)));
            changedPath = op.path;
          }
          var changedNode = index_es.Node.get(editor, index_es.Path.parent(changedPath));
          var changedNodeKey = DOMEditor.findKey(e, changedNode);
          var changedPathRef = index_es.Editor.pathRef(e, index_es.Path.parent(changedPath));
    ...
    for (var [path, key] of matches) {
      var [node] = index_es.Editor.node(e, path);
      NODE_TO_KEY.set(node, key);
    }
    for (var [pathRef, _key] of pathRefMatches) {
      if (pathRef.current) {
        var [_node] = index_es.Editor.node(e, pathRef.current);
  ...
  e.setFragmentData = data => {
    ...
    var [start, end] = index_es.Range.edges(selection);
    var startVoid = index_es.Editor.void(e, {
      at: start.path
    });
    var endVoid = index_es.Editor.void(e, {
      at: end.path
    });
    if (index_es.Range.isCollapsed(selection) && !startVoid) {
      return;
    }
  ...
  e.insertTextData = data => {
    ...
    if (text) {
      ...
      for (var line of lines) {
        if (split) {
          index_es.Transforms.splitNodes(e, {
            always: true
          });
        }
        e.insertText(line);
...
var getMatches = (e, path) => {
  var matches = [];
  for (var [n, p] of index_es.Editor.levels(e, {
    at: path
  })) {
...
var create = key => {
  ...
  var isGeneric = generic && index.libExports.isHotkey(generic);
  var isApple = apple && index.libExports.isHotkey(apple);
  var isWindows = windows && index.libExports.isHotkey(windows);
  return event => {
...
var isElementDecorationsEqual = (list, another) => {
  ...
  for (var i = 0; i < list.length; i++) {
    var range = list[i];
    var other = another[i];
    if (!index_es.Range.equals(range, other) || !isDecorationFlagsEqual(range, other)) {
      return false;
    }
  }
  return true;
...
var splitDecorationsByChild = (editor, node, decorations) => {
  ...
  var ancestorRange = index_es.Editor.range(editor, path);
  var cachedChildRanges = new Array(node.children.length);
  var getChildRange = index => {
    var cachedRange = cachedChildRanges[index];
    if (cachedRange) return cachedRange;
    var childRange = index_es.Editor.range(editor, [...path, index]);
    cachedChildRanges[index] = childRange;
    return childRange;
  };
  for (var decoration of decorations) {
    var decorationRange = index_es.Range.intersection(ancestorRange, decoration);
    if (!decorationRange) continue;
    var [startPoint, endPoint] = index_es.Range.edges(decorationRange);
    ...
    for (var i = startIndex; i <= endIndex; i++) {
      ...
      var childDecorationRange = index_es.Range.intersection(childRange, decoration);
