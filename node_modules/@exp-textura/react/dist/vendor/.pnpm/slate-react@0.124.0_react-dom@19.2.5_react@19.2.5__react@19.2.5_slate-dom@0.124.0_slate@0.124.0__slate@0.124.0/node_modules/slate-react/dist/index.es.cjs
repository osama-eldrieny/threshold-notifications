...
const debounce = require('../../../../../../_virtual/debounce.cjs');
const throttle = require('../../../../../../_virtual/throttle.cjs');
const React = require('react');
const index$1 = require('../../../../scroll-into-view-if-needed@3.1.0/node_modules/scroll-into-view-if-needed/dist/index.cjs');
const index_es$1 = require('../../../../slate@0.124.0/node_modules/slate/dist/index.es.cjs');
const index_es = require('../../../../slate-dom@0.124.0_slate@0.124.0/node_modules/slate-dom/dist/index.es.cjs');
const ResizeObserver = require('../../../../@juggle_resize-observer@3.4.0/node_modules/@juggle/resize-observer/lib/ResizeObserver.cjs');
require('../../../../@juggle_resize-observer@3.4.0/node_modules/@juggle/resize-observer/lib/algorithms/calculateBoxSize.cjs');
...
var ReactEditor = index_es.DOMEditor;
...
function createAndroidInputManager(_ref) {
  ...
  var applyPendingSelection = () => {
    var pendingSelection = index_es.EDITOR_TO_PENDING_SELECTION.get(editor);
    index_es.EDITOR_TO_PENDING_SELECTION.delete(editor);
    if (pendingSelection) {
      var {
        selection
      } = editor;
      var normalized = index_es.normalizeRange(editor, pendingSelection);
      if (normalized && (!selection || !index_es$1.Range.equals(normalized, selection))) {
        index_es$1.Transforms.select(editor, normalized);
      }
    }
  };
  var performAction = () => {
    var action = index_es.EDITOR_TO_PENDING_ACTION.get(editor);
    index_es.EDITOR_TO_PENDING_ACTION.delete(editor);
    if (!action) {
      return;
    }
    if (action.at) {
      var target = index_es$1.Location.isPoint(action.at) ? index_es.normalizePoint(editor, action.at) : index_es.normalizeRange(editor, action.at);
      if (!target) {
        return;
      }
      var _targetRange = index_es$1.Editor.range(editor, target);
      if (!editor.selection || !index_es$1.Range.equals(editor.selection, _targetRange)) {
        index_es$1.Transforms.select(editor, target);
      }
    }
    action.run();
  };
  var flush = () => {
    ...
    var selectionRef = editor.selection && index_es$1.Editor.rangeRef(editor, editor.selection, {
      affinity: 'forward'
    });
    index_es.EDITOR_TO_USER_MARKS.set(editor, editor.marks);
    debug('flush', index_es.EDITOR_TO_PENDING_ACTION.get(editor), index_es.EDITOR_TO_PENDING_DIFFS.get(editor));
    var scheduleSelectionChange = hasPendingDiffs();
    var diff;
    while (diff = (_EDITOR_TO_PENDING_DI = index_es.EDITOR_TO_PENDING_DIFFS.get(editor)) === null || _EDITOR_TO_PENDING_DI === void 0 ? void 0 : _EDITOR_TO_PENDING_DI[0]) {
      var _EDITOR_TO_PENDING_DI, _EDITOR_TO_PENDING_DI2;
      var pendingMarks = index_es.EDITOR_TO_PENDING_INSERTION_MARKS.get(editor);
      if (pendingMarks !== undefined) {
        index_es.EDITOR_TO_PENDING_INSERTION_MARKS.delete(editor);
        editor.marks = pendingMarks;
      }
      if (pendingMarks && insertPositionHint === false) {
        insertPositionHint = null;
      }
      var range = index_es.targetRange(diff);
      if (!editor.selection || !index_es$1.Range.equals(editor.selection, range)) {
        index_es$1.Transforms.select(editor, range);
      }
      if (diff.diff.text) {
        index_es$1.Editor.insertText(editor, diff.diff.text);
      } else {
        index_es$1.Editor.deleteFragment(editor);
      }
      // Remove diff only after we have applied it to account for it when transforming
      // pending ranges.
      index_es.EDITOR_TO_PENDING_DIFFS.set(editor, (_EDITOR_TO_PENDING_DI2 = index_es.EDITOR_TO_PENDING_DIFFS.get(editor)) === null || _EDITOR_TO_PENDING_DI2 === void 0 ? void 0 : _EDITOR_TO_PENDING_DI2.filter(_ref2 => {
        var {
          id
        } = _ref2;
        return id !== diff.id;
      }));
      if (!index_es.verifyDiffState(editor, diff)) {
        scheduleSelectionChange = false;
        index_es.EDITOR_TO_PENDING_ACTION.delete(editor);
        index_es.EDITOR_TO_USER_MARKS.delete(editor);
        flushing = 'action';
    ...
    if (selection && !index_es.EDITOR_TO_PENDING_SELECTION.get(editor) && (!editor.selection || !index_es$1.Range.equals(selection, editor.selection))) {
      index_es$1.Transforms.select(editor, selection);
    }
    if (hasPendingAction()) {
      performAction();
      return;
    ...
    applyPendingSelection();
    var userMarks = index_es.EDITOR_TO_USER_MARKS.get(editor);
    index_es.EDITOR_TO_USER_MARKS.delete(editor);
  ...
  var handleCompositionEnd = _event => {
    ...
    compositionEndTimeoutId = setTimeout(() => {
      index_es.IS_COMPOSING.set(editor, false);
      flush();
    }, RESOLVE_DELAY);
  };
  var handleCompositionStart = _event => {
    index_es.IS_COMPOSING.set(editor, true);
    if (compositionEndTimeoutId) {
      clearTimeout(compositionEndTimeoutId);
  ...
  var updatePlaceholderVisibility = function updatePlaceholderVisibility() {
    ...
    var placeholderElement = index_es.EDITOR_TO_PLACEHOLDER_ELEMENT.get(editor);
  ...
  var storeDiff = (path, diff) => {
    ...
    var pendingDiffs = (_EDITOR_TO_PENDING_DI3 = index_es.EDITOR_TO_PENDING_DIFFS.get(editor)) !== null && _EDITOR_TO_PENDING_DI3 !== void 0 ? _EDITOR_TO_PENDING_DI3 : [];
    index_es.EDITOR_TO_PENDING_DIFFS.set(editor, pendingDiffs);
    var target = index_es$1.Node.leaf(editor, path);
    var idx = pendingDiffs.findIndex(change => index_es$1.Path.equals(change.path, path));
    if (idx < 0) {
      var normalized = index_es.normalizeStringDiff(target.text, diff);
      if (normalized) {
        pendingDiffs.push({
          path,
          diff,
    ...
    var merged = index_es.mergeStringDiffs(target.text, pendingDiffs[idx].diff, diff);
    if (!merged) {
  ...
  var scheduleAction = function scheduleAction(run) {
    ...
    index_es.EDITOR_TO_PENDING_ACTION.set(editor, {
      at,
      run
    });
  ...
  var handleDOMBeforeInput = event => {
    ...
    if (index_es.IS_NODE_MAP_DIRTY.get(editor)) {
      return;
    }
    var {
      inputType: type
    ...
    if (type.startsWith('delete')) {
      ...
      var [start, end] = index_es$1.Range.edges(targetRange);
      var [leaf, path] = index_es$1.Editor.leaf(editor, start.path);
      if (index_es$1.Range.isExpanded(targetRange)) {
        if (leaf.text.length === start.offset && end.offset === 0) {
          var next = index_es$1.Editor.next(editor, {
            at: start.path,
            match: index_es$1.Node.isText
          });
          if (next && index_es$1.Path.equals(next[1], end.path)) {
      ...
      var pendingDiffs = index_es.EDITOR_TO_PENDING_DIFFS.get(editor);
      var relevantPendingDiffs = pendingDiffs === null || pendingDiffs === void 0 ? void 0 : pendingDiffs.find(change => index_es$1.Path.equals(change.path, path));
      var diffs = relevantPendingDiffs ? [relevantPendingDiffs.diff, diff] : [diff];
      var text = index_es.applyStringDiff(leaf.text, ...diffs);
      if (text.length === 0) {
      ...
      if (index_es$1.Range.isExpanded(targetRange)) {
        if (canStoreDiff && index_es$1.Path.equals(targetRange.anchor.path, targetRange.focus.path)) {
          var point = {
            path: targetRange.anchor.path,
            offset: start.offset
          };
          var range = index_es$1.Editor.range(editor, point, point);
        ...
        return scheduleAction(() => index_es$1.Editor.deleteFragment(editor, {
          direction
        }), {
          at: targetRange
        });
    ...
    switch (type) {
      ...
      case 'deleteContentForward':
        {
          ...
          if (canStoreDiff && index_es$1.Range.isCollapsed(targetRange)) {
            var targetNode = index_es$1.Node.leaf(editor, anchor.path);
          ...
          return scheduleAction(() => index_es$1.Editor.deleteForward(editor), {
            at: targetRange
          });
        }
      case 'deleteContentBackward':
        {
          ...
          var nativeCollapsed = index_es.isDOMSelection(nativeTargetRange) ? nativeTargetRange.isCollapsed : !!((_nativeTargetRange = nativeTargetRange) !== null && _nativeTargetRange !== void 0 && _nativeTargetRange.collapsed);
          if (canStoreDiff && nativeCollapsed && index_es$1.Range.isCollapsed(targetRange) && _anchor.offset > 0) {
          ...
          return scheduleAction(() => index_es$1.Editor.deleteBackward(editor), {
            at: targetRange
          });
        }
      case 'deleteEntireSoftLine':
        {
          return scheduleAction(() => {
            index_es$1.Editor.deleteBackward(editor, {
              unit: 'line'
            });
            index_es$1.Editor.deleteForward(editor, {
              unit: 'line'
            });
          }, {
            at: targetRange
      ...
      case 'insertLineBreak':
        {
          return scheduleAction(() => index_es$1.Editor.insertSoftBreak(editor), {
            at: targetRange
          });
        }
      case 'insertParagraph':
        {
          return scheduleAction(() => index_es$1.Editor.insertBreak(editor), {
            at: targetRange
          });
        }
      ...
      case 'insertText':
        {
          ...
          if (index_es.EDITOR_TO_PENDING_INSERTION_MARKS.get(editor)) {
            _text = _text.replace('\uFEFF', '');
          }
          ...
          if (_text.includes('\n')) {
            return scheduleAction(() => {
              ...
              parts.forEach((line, i) => {
                if (line) {
                  index_es$1.Editor.insertText(editor, line);
                }
                if (i !== parts.length - 1) {
                  index_es$1.Editor.insertSoftBreak(editor);
                }
              });
            }, {
              at: targetRange
          ...
          if (index_es$1.Path.equals(targetRange.anchor.path, targetRange.focus.path)) {
            var [_start, _end] = index_es$1.Range.edges(targetRange);
            var _diff = {
              start: _start.offset,
              end: _end.offset,
              text: _text
            ...
            if (_text && insertPositionHint && type === 'insertCompositionText') {
              ...
              } else if (insertPositionHint && index_es$1.Range.isCollapsed(targetRange) && insertPositionHint.end + insertPositionHint.text.length === _start.offset) {
            ...
            if (canStoreDiff) {
              ...
              if (currentSelection) {
                ...
                scheduleAction(() => {
                  index_es$1.Transforms.select(editor, {
                    anchor: newPoint,
                    focus: newPoint
                  });
                }, {
          ...
          return scheduleAction(() => index_es$1.Editor.insertText(editor, _text), {
            at: targetRange
          });
        }
    }
  ...
  var hasPendingAction = () => {
    return !!index_es.EDITOR_TO_PENDING_ACTION.get(editor);
  };
  var hasPendingDiffs = () => {
    var _EDITOR_TO_PENDING_DI4;
    return !!((_EDITOR_TO_PENDING_DI4 = index_es.EDITOR_TO_PENDING_DIFFS.get(editor)) !== null && _EDITOR_TO_PENDING_DI4 !== void 0 && _EDITOR_TO_PENDING_DI4.length);
  };
  ...
  var handleUserSelect = range => {
    index_es.EDITOR_TO_PENDING_SELECTION.set(editor, range);
    if (flushTimeoutId) {
      clearTimeout(flushTimeoutId);
      flushTimeoutId = null;
    }
    ...
    var pathChanged = !selection || !index_es$1.Path.equals(selection.anchor.path, range.anchor.path);
    var parentPathChanged = !selection || !index_es$1.Path.equals(selection.anchor.path.slice(0, -1), range.anchor.path.slice(0, -1));
  ...
  var handleDomMutations = mutations => {
    ...
    if (mutations.some(mutation => index_es.isTrackedMutation(editor, mutation, mutations))) {
      var _EDITOR_TO_FORCE_REND;
      // Cause a re-render to restore the dom state if we encounter tracked mutations without
      // a corresponding pending action.
      (_EDITOR_TO_FORCE_REND = index_es.EDITOR_TO_FORCE_RENDER.get(editor)) === null || _EDITOR_TO_FORCE_REND === void 0 || _EDITOR_TO_FORCE_REND();
    }
  };
  return {
    flush,
 ...
 */
var useIsomorphicLayoutEffect = index_es.CAN_USE_DOM ? React.useLayoutEffect : React.useEffect;
...
var useAndroidInputManager = !index_es.IS_ANDROID ? () => null : _ref => {
  var {
      node
    } = _ref,
    options = _objectWithoutProperties(_ref, _excluded$2);
  if (!index_es.IS_ANDROID) {
    return null;
  }
  var editor = useSlateStatic();
  var isMounted = useIsMounted();
  var [inputManager] = React.useState(() => createAndroidInputManager(_objectSpread$7({
    editor
  }, options)));
  useMutationObserver(node, inputManager.handleDomMutations, MUTATION_OBSERVER_CONFIG$1);
  index_es.EDITOR_TO_SCHEDULE_FLUSH.set(editor, inputManager.scheduleFlush);
  if (isMounted) {
    inputManager.flush();
  }
  return inputManager;
...
var String$1 = props => {
  ...
  var parentPath = index_es$1.Path.parent(path);
  var isMarkPlaceholder = Boolean(leaf[index_es.MARK_PLACEHOLDER_SYMBOL]);
  // COMPAT: Render text inside void nodes with a zero-width space.
  // So the node can contain selection but the text is not visible.
  if (editor.isVoid(parent)) {
    return /*#__PURE__*/React.createElement(ZeroWidthString, {
      length: index_es$1.Node.string(parent).length
    });
  }
  // COMPAT: If this is the last text node in an empty block, render a zero-
  // width space that will convert into a line break when copying and pasting
  // to support expected plain text.
  if (leaf.text === '' && parent.children[parent.children.length - 1] === text && !editor.isInline(parent) && index_es$1.Editor.string(editor, parentPath) === '') {
    return /*#__PURE__*/React.createElement(ZeroWidthString, {
      isLineBreak: true,
...
var ZeroWidthString = props => {
  ...
  return /*#__PURE__*/React.createElement("span", _objectSpread$6({}, attributes), !index_es.IS_ANDROID || !isLineBreak ? '\uFEFF' : null, isLineBreak ? /*#__PURE__*/React.createElement("br", null) : null);
};
...
var PLACEHOLDER_DELAY = index_es.IS_ANDROID ? 300 : 0;
...
var Leaf = props => {
  ...
  var callbackPlaceholderRef = React.useCallback(placeholderEl => {
    ...
    if (placeholderEl == null) {
      ...
      index_es.EDITOR_TO_PLACEHOLDER_ELEMENT.delete(editor);
      (_leaf$onPlaceholderRe = leaf.onPlaceholderResize) === null || _leaf$onPlaceholderRe === void 0 || _leaf$onPlaceholderRe.call(leaf, null);
    } else {
      index_es.EDITOR_TO_PLACEHOLDER_ELEMENT.set(editor, placeholderEl);
      if (!placeholderResizeObserver.current) {
        // Create a new observer and observe the placeholder element.
        var ResizeObserver$1 = window.ResizeObserver || ResizeObserver.ResizeObserver;
  ...
  var leafIsPlaceholder = Boolean(leaf[index_es.PLACEHOLDER_SYMBOL]);
  React.useEffect(() => {
    ...
    var placeholderProps = {
      ...
      attributes: {
        ...
        style: {
          ...
          WebkitUserModify: index_es.IS_WEBKIT ? 'inherit' : undefined
        },
...
var MemoizedLeaf = /*#__PURE__*/React.memo(Leaf, (prev, next) => {
  return next.parent === prev.parent && next.isLast === prev.isLast && next.renderLeaf === prev.renderLeaf && next.renderPlaceholder === prev.renderPlaceholder && next.text === prev.text && index_es$1.Text.equals(next.leaf, prev.leaf) && next.leaf[index_es.PLACEHOLDER_SYMBOL] === prev.leaf[index_es.PLACEHOLDER_SYMBOL];
});
...
function useGenericSelector(selector, equalityFn) {
  ...
  var latestSelector = React.useRef(() => null);
...
var useDecorations = (node, parentDecorations) => {
  ...
  var {
    decorate,
    addEventListener
  } = React.useContext(DecorateContext);
  // Not memoized since we want nodes to be decorated on each render
  var selector = () => {
    var path = ReactEditor.findPath(editor, node);
    return decorate([node, path]);
  };
  var equalityFn = index_es$1.Node.isText(node) ? index_es.isTextDecorationsEqual : index_es.isElementDecorationsEqual;
  ...
  return React.useMemo(() => [...decorations, ...parentDecorations], [decorations, parentDecorations]);
};
var useDecorateContext = decorateProp => {
  ...
  var decorate = React.useCallback(entry => latestDecorate.current(entry), []);
  var addEventListener = React.useCallback(callback => {
...
var Text = props => {
  ...
  var decoratedLeaves = index_es$1.Text.decorations(text, decorations);
  ...
  var callbackRef = React.useCallback(span => {
    var KEY_TO_ELEMENT = index_es.EDITOR_TO_KEY_TO_ELEMENT.get(editor);
    if (span) {
      KEY_TO_ELEMENT === null || KEY_TO_ELEMENT === void 0 || KEY_TO_ELEMENT.set(key, span);
      index_es.NODE_TO_ELEMENT.set(text, span);
      index_es.ELEMENT_TO_NODE.set(span, text);
    } else {
      KEY_TO_ELEMENT === null || KEY_TO_ELEMENT === void 0 || KEY_TO_ELEMENT.delete(key);
      index_es.NODE_TO_ELEMENT.delete(text);
      if (ref.current) {
        index_es.ELEMENT_TO_NODE.delete(ref.current);
      }
    }
    ref.current = span;
  }, [ref, editor, key, text]);
...
var MemoizedText = /*#__PURE__*/React.memo(Text, (prev, next) => {
  return next.parent === prev.parent && next.isLast === prev.isLast && next.renderText === prev.renderText && next.renderLeaf === prev.renderLeaf && next.renderPlaceholder === prev.renderPlaceholder && next.text === prev.text && index_es.isTextDecorationsEqual(next.decorations, prev.decorations);
});
...
var Element = props => {
  ...
  var ref = React.useCallback(ref => {
    ...
    if (ref) {
      ...
      index_es.NODE_TO_ELEMENT.set(element, ref);
      index_es.ELEMENT_TO_NODE.set(ref, element);
    } else {
      KEY_TO_ELEMENT === null || KEY_TO_ELEMENT === void 0 || KEY_TO_ELEMENT.delete(key);
      index_es.NODE_TO_ELEMENT.delete(element);
    }
  }, [editor, key, element]);
  ...
  // for text direction.
  if (!isInline && index_es$1.Editor.hasInlines(editor, element)) {
    var text = index_es$1.Node.string(element);
    var dir = index.default(text);
    if (dir === 'rtl') {
      attributes.dir = dir;
    }
  }
  // If it's a void node, wrap the children in extra void-specific elements.
  if (index_es$1.Editor.isVoid(editor, element)) {
    attributes['data-slate-void'] = true;
    if (!readOnly && isInline) {
      attributes.contentEditable = false;
    }
    var Tag = isInline ? 'span' : 'div';
    var [[_text]] = index_es$1.Node.texts(element);
    ...
    index_es.NODE_TO_INDEX.set(_text, 0);
    index_es.NODE_TO_PARENT.set(_text, element);
  }
...
var MemoizedElement = /*#__PURE__*/React.memo(Element, (prev, next) => {
  return prev.element === next.element && prev.renderElement === next.renderElement && prev.renderChunk === next.renderChunk && prev.renderText === next.renderText && prev.renderLeaf === next.renderLeaf && prev.renderPlaceholder === next.renderPlaceholder && index_es.isElementDecorationsEqual(prev.decorations, next.decorations);
});
/**
...
class ChunkTreeHelper {
  ...
  validateState() {
    ...
    if (!index_es$1.Path.equals(this.pointerIndexStack, actualIndexStack)) {
...
var useChildren = props => {
  ...
  var editor = useSlateStatic();
  index_es.IS_NODE_MAP_DIRTY.set(editor, false);
  var isBlock = index_es$1.Node.isElement(node) && !editor.isInline(node);
  var isLeafBlock = isBlock && index_es$1.Editor.hasInlines(editor, node);
  ...
  if (!chunking) {
    node.children.forEach((n, i) => {
      index_es.NODE_TO_INDEX.set(n, i);
      index_es.NODE_TO_PARENT.set(n, node);
    });
  }
  var renderElementComponent = React.useCallback((n, i, cachedKey) => {
  ...
  if (!chunking) {
    return node.children.map((n, i) => index_es$1.Node.isText(n) ? renderTextComponent(n, i) : renderElementComponent(n, i));
  }
...
var useDecorationsByChild = (editor, node, decorations) => {
  var decorationsByChild = index_es.splitDecorationsByChild(editor, node, decorations);
  ...
  for (var i = 0; i < decorationsByChild.length; i++) {
    ...
    if (!index_es.isElementDecorationsEqual(previousDecorations, _decorations)) {
...
function useSelectorContext() {
  ...
  var flushDeferred = React.useCallback(() => {
...
function useFlushDeferredSelectorsOnRender() {
  var {
    flushDeferred
  } = React.useContext(SlateSelectorContext);
...
var createRestoreDomManager = (editor, receivedUserInput) => {
  ...
  var registerMutations = mutations => {
    ...
    var trackedMutations = mutations.filter(mutation => index_es.isTrackedMutation(editor, mutation, mutations));
...
var RestoreDOM = index_es.IS_ANDROID ? RestoreDOMComponent : _ref => {
  var {
    children
  } = _ref;
...
var Editable = /*#__PURE__*/React.forwardRef((props, forwardedRef) => {
  var defaultRenderPlaceholder = React.useCallback(props => /*#__PURE__*/React.createElement(DefaultPlaceholder, _objectSpread({}, props)), []);
  var {
      autoFocus,
  ...
  var [, forceRender] = React.useReducer(s => s + 1, 0);
  index_es.EDITOR_TO_FORCE_RENDER.set(editor, forceRender);
  // Update internal state on each render.
  index_es.IS_READ_ONLY.set(editor, readOnly);
  ...
  var onDOMSelectionChange = React.useMemo(() => throttle.default(() => {
    if (index_es.IS_NODE_MAP_DIRTY.get(editor)) {
      onDOMSelectionChange();
      return;
    }
    ...
    if (!processing.current && index_es.IS_WEBKIT && root instanceof ShadowRoot) {
      processing.current = true;
      var active = index_es.getActiveElement();
      if (active) {
        document.execCommand('indent');
      } else {
        index_es$1.Transforms.deselect(editor);
      }
      processing.current = false;
      return;
    }
    var androidInputManager = androidInputManagerRef.current;
    if ((index_es.IS_ANDROID || !ReactEditor.isComposing(editor)) && (!state.isUpdatingSelection || androidInputManager !== null && androidInputManager !== void 0 && androidInputManager.isFlushing()) && !state.isDraggingInternally) {
      var _root = ReactEditor.findDocumentOrShadowRoot(editor);
      var {
        activeElement
      } = _root;
      var _el = ReactEditor.toDOMNode(editor, editor);
      var domSelection = index_es.getSelection(_root);
      if (activeElement === _el) {
        state.latestElement = activeElement;
        index_es.IS_FOCUSED.set(editor, true);
      } else {
        index_es.IS_FOCUSED.delete(editor);
      }
      if (!domSelection) {
        return index_es$1.Transforms.deselect(editor);
      }
      var {
        anchorNode,
        focusNode
  ...
  var scheduleOnDOMSelectionChange = React.useMemo(() => debounce.default(onDOMSelectionChange, 0), [onDOMSelectionChange]);
  ...
  useIsomorphicLayoutEffect(() => {
    ...
    // Update element-related weak maps with the DOM element ref.
    var window;
    if (ref.current && (window = index_es.getDefaultView(ref.current))) {
      index_es.EDITOR_TO_WINDOW.set(editor, window);
      index_es.EDITOR_TO_ELEMENT.set(editor, ref.current);
      index_es.NODE_TO_ELEMENT.set(editor, ref.current);
      index_es.ELEMENT_TO_NODE.set(ref.current, editor);
    } else {
      index_es.NODE_TO_ELEMENT.delete(editor);
    }
    ...
    var domSelection = index_es.getSelection(root);
    ...
    var setDomSelection = forceChange => {
      ...
      if (index_es.IS_FIREFOX && domSelection.rangeCount > 1) {
      ...
      var editorElement = index_es.EDITOR_TO_ELEMENT.get(editor);
      var hasDomSelectionInEditor = false;
      if (index_es.containsShadowAware(editorElement, anchorNode) && index_es.containsShadowAware(editorElement, focusNode)) {
      ...
      if (hasDomSelection && hasDomSelectionInEditor && selection && !forceChange) {
        ...
        if (slateRange && index_es$1.Range.equals(slateRange, selection)) {
          var _anchorNode;
      ...
      if (newDomRange) {
        if (ReactEditor.isComposing(editor) && !index_es.IS_ANDROID) {
          domSelection.collapseToEnd();
        } else if (index_es$1.Range.isBackward(selection)) {
    ...
    if (!index_es.IS_ANDROID || !ensureSelection) {
      setTimeout(() => {
  ...
  var onDOMBeforeInput = React.useCallback(event => {
    ...
    var el = ReactEditor.toDOMNode(editor, editor);
    var root = el.getRootNode();
    if (processing !== null && processing !== void 0 && processing.current && index_es.IS_WEBKIT && root instanceof ShadowRoot) {
      ...
      index_es$1.Transforms.select(editor, slateRange);
    ...
    if (!readOnly && ReactEditor.hasEditableTarget(editor, event.target) && !isDOMEventHandled(event, propsOnDOMBeforeInput)) {
      ...
      var native = false;
      if (type === 'insertText' && selection && index_es$1.Range.isCollapsed(selection) &&
      ...
      selection.anchor.offset !== 0) {
        ...
        if (!index_es.IS_NODE_MAP_DIRTY.get(editor)) {
          ...
          if (native && node.parentElement && (_window === null || _window === void 0 || (_window$getComputedSt = _window.getComputedStyle(node.parentElement)) === null || _window$getComputedSt === void 0 ? void 0 : _window$getComputedSt.whiteSpace) === 'pre') {
            var block = index_es$1.Editor.above(editor, {
              at: anchor.path,
              match: n => index_es$1.Node.isElement(n) && index_es$1.Editor.isBlock(editor, n)
            });
            if (block && index_es$1.Node.string(block[0]).includes('\t')) {
              native = false;
            }
          }
        }
      ...
      if ((!type.startsWith('delete') || type.startsWith('deleteBy')) && !index_es.IS_NODE_MAP_DIRTY.get(editor)) {
        ...
        if (targetRange) {
          ...
          if (!selection || !index_es$1.Range.equals(selection, _range)) {
            native = false;
            var selectionRef = !isCompositionChange && editor.selection && index_es$1.Editor.rangeRef(editor, editor.selection);
            index_es$1.Transforms.select(editor, _range);
            if (selectionRef) {
              index_es.EDITOR_TO_USER_SELECTION.set(editor, selectionRef);
            }
          }
        }
      }
      ...
      if (selection && index_es$1.Range.isExpanded(selection) && type.startsWith('delete')) {
        var direction = type.endsWith('Backward') ? 'backward' : 'forward';
        index_es$1.Editor.deleteFragment(editor, {
          direction
        });
      ...
      switch (type) {
        ...
        case 'insertParagraph':
          {
            index_es$1.Editor.insertBreak(editor);
            break;
          }
        case 'insertFromComposition':
        case 'insertFromDrop':
        ...
        case 'insertText':
          {
            ...
            if ((data === null || data === void 0 ? void 0 : data.constructor.name) === 'DataTransfer') {
              ...
              if (native) {
                deferredOperations.current.push(() => index_es$1.Editor.insertText(editor, data));
              } else {
                index_es$1.Editor.insertText(editor, data);
              }
            }
            break;
          }
      ...
      var toRestore = (_EDITOR_TO_USER_SELEC = index_es.EDITOR_TO_USER_SELECTION.get(editor)) === null || _EDITOR_TO_USER_SELEC === void 0 ? void 0 : _EDITOR_TO_USER_SELEC.unref();
      index_es.EDITOR_TO_USER_SELECTION.delete(editor);
      if (toRestore && (!editor.selection || !index_es$1.Range.equals(editor.selection, toRestore))) {
        index_es$1.Transforms.select(editor, toRestore);
      }
    }
  ...
  var callbackRef = React.useCallback(node => {
    if (node == null) {
      onDOMSelectionChange.cancel();
      scheduleOnDOMSelectionChange.cancel();
      index_es.EDITOR_TO_ELEMENT.delete(editor);
      index_es.NODE_TO_ELEMENT.delete(editor);
      if (ref.current && index_es.HAS_BEFORE_INPUT_SUPPORT) {
      ...
      // https://github.com/facebook/react/issues/11211
      if (index_es.HAS_BEFORE_INPUT_SUPPORT) {
        // @ts-ignore The `beforeinput` event isn't recognized.
  ...
  var showPlaceholder = placeholder && editor.children.length === 1 && Array.from(index_es$1.Node.texts(editor)).length === 1 && index_es$1.Node.string(editor) === '' && !isComposing;
  ...
  if (showPlaceholder) {
    var start = index_es$1.Editor.start(editor, []);
    decorations.push({
      [index_es.PLACEHOLDER_SYMBOL]: true,
      placeholder,
  ...
  if (editor.selection && index_es$1.Range.isCollapsed(editor.selection) && marks) {
    var {
      anchor
    } = editor.selection;
    var leaf = index_es$1.Node.leaf(editor, anchor.path);
    var {
        text
      } = leaf,
  ...
  React.useEffect(() => {
    setTimeout(() => {
      ...
      if (selection) {
        ...
        var _text = index_es$1.Node.leaf(editor, _anchor.path);
        // While marks isn't a 'complete' text, we can still use loose Text.equals
        // here which only compares marks anyway.
        if (marks && !index_es$1.Text.equals(_text, marks, {
          loose: true
        })) {
          index_es.EDITOR_TO_PENDING_INSERTION_MARKS.set(editor, marks);
          return;
        }
      }
      index_es.EDITOR_TO_PENDING_INSERTION_MARKS.delete(editor);
  ...
  return /*#__PURE__*/React.createElement(ReadOnlyContext.Provider, {
    ...
    spellCheck: index_es.HAS_BEFORE_INPUT_SUPPORT || !index_es.CAN_USE_DOM ? attributes.spellCheck : false,
    autoCorrect: index_es.HAS_BEFORE_INPUT_SUPPORT || !index_es.CAN_USE_DOM ? attributes.autoCorrect : 'false',
    autoCapitalize: index_es.HAS_BEFORE_INPUT_SUPPORT || !index_es.CAN_USE_DOM ? attributes.autoCapitalize : 'false',
    ...
    onBeforeInput: React.useCallback(event => {
      ...
      if (!index_es.HAS_BEFORE_INPUT_SUPPORT && !readOnly && !isEventHandled(event, attributes.onBeforeInput) && ReactEditor.hasSelectableTarget(editor, event.target)) {
        event.preventDefault();
        if (!ReactEditor.isComposing(editor)) {
          var _text2 = event.data;
          index_es$1.Editor.insertText(editor, _text2);
        }
      }
    ...
    onBlur: React.useCallback(event => {
      ...
      if (index_es.isDOMElement(relatedTarget) && relatedTarget.hasAttribute('data-slate-spacer')) {
        return;
      }
      // COMPAT: The event should be ignored if the focus is moving to a
      // non- editable section of an element that isn't a void node (eg.
      // a list item of the check list example).
      if (relatedTarget != null && index_es.isDOMNode(relatedTarget) && ReactEditor.hasDOMNode(editor, relatedTarget)) {
        var node = ReactEditor.toSlateNode(editor, relatedTarget);
        if (index_es$1.Node.isElement(node) && !editor.isVoid(node)) {
          return;
        }
      }
    ...
    onClick: React.useCallback(event => {
      if (ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onClick) && index_es.isDOMNode(event.target)) {
        ...
        if (!index_es$1.Editor.hasPath(editor, path) || index_es$1.Node.get(editor, path) !== node) {
          return;
        }
        if (event.detail === index_es.TRIPLE_CLICK && path.length >= 1) {
          var blockPath = path;
          if (!(index_es$1.Node.isElement(node) && index_es$1.Editor.isBlock(editor, node))) {
            var _block$;
            var block = index_es$1.Editor.above(editor, {
              match: n => index_es$1.Node.isElement(n) && index_es$1.Editor.isBlock(editor, n),
              at: path
          ...
          var range = index_es$1.Editor.range(editor, blockPath);
          index_es$1.Transforms.select(editor, range);
          return;
        }
        if (readOnly) {
        ...
        var _start = index_es$1.Editor.start(editor, path);
        var end = index_es$1.Editor.end(editor, path);
        var startVoid = index_es$1.Editor.void(editor, {
          at: _start
        });
        var endVoid = index_es$1.Editor.void(editor, {
          at: end
        });
        if (startVoid && endVoid && index_es$1.Path.equals(startVoid[1], endVoid[1])) {
          var _range2 = index_es$1.Editor.range(editor, _start);
          index_es$1.Transforms.select(editor, _range2);
        }
      }
    ...
    onCompositionEnd: React.useCallback(event => {
      ...
      if (ReactEditor.hasSelectableTarget(editor, event.target)) {
        ...
        if (isEventHandled(event, attributes.onCompositionEnd) || index_es.IS_ANDROID) {
          return;
        }
        ...
        if (!index_es.IS_WEBKIT && !index_es.IS_FIREFOX_LEGACY && !index_es.IS_IOS && !index_es.IS_WECHATBROWSER && !index_es.IS_UC_MOBILE && event.data) {
          var placeholderMarks = index_es.EDITOR_TO_PENDING_INSERTION_MARKS.get(editor);
          index_es.EDITOR_TO_PENDING_INSERTION_MARKS.delete(editor);
          ...
          index_es$1.Editor.insertText(editor, event.data);
          var userMarks = index_es.EDITOR_TO_USER_MARKS.get(editor);
          index_es.EDITOR_TO_USER_MARKS.delete(editor);
          if (userMarks !== undefined) {
            editor.marks = userMarks;
    ...
    onCompositionStart: React.useCallback(event => {
      ...
      if (ReactEditor.hasSelectableTarget(editor, event.target)) {
        ...
        if (isEventHandled(event, attributes.onCompositionStart) || index_es.IS_ANDROID) {
          return;
        }
        setIsComposing(true);
        var {
        ...
        if (selection && index_es$1.Range.isExpanded(selection)) {
          index_es$1.Editor.deleteFragment(editor);
          return;
        }
      }
    ...
    onCut: React.useCallback(event => {
      if (!readOnly && ReactEditor.hasSelectableTarget(editor, event.target) && !isEventHandled(event, attributes.onCut) && !isDOMEventTargetInput(event)) {
        ...
        if (selection) {
          if (index_es$1.Range.isExpanded(selection)) {
            index_es$1.Editor.deleteFragment(editor);
          } else {
            var node = index_es$1.Node.parent(editor, selection.anchor.path);
            if (index_es$1.Editor.isVoid(editor, node)) {
              index_es$1.Transforms.delete(editor);
            }
          }
        }
      }
    ...
    onDragOver: React.useCallback(event => {
      if (ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onDragOver)) {
        ...
        var node = ReactEditor.toSlateNode(editor, event.target);
        if (index_es$1.Node.isElement(node) && index_es$1.Editor.isVoid(editor, node)) {
          event.preventDefault();
        }
      }
    ...
    onDragStart: React.useCallback(event => {
      if (!readOnly && ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onDragStart)) {
        var node = ReactEditor.toSlateNode(editor, event.target);
        var path = ReactEditor.findPath(editor, node);
        var voidMatch = index_es$1.Node.isElement(node) && index_es$1.Editor.isVoid(editor, node) || index_es$1.Editor.void(editor, {
          at: path,
          voids: true
        });
        ...
        if (voidMatch) {
          var range = index_es$1.Editor.range(editor, path);
          index_es$1.Transforms.select(editor, range);
        }
    ...
    onDrop: React.useCallback(event => {
      if (!readOnly && ReactEditor.hasTarget(editor, event.target) && !isEventHandled(event, attributes.onDrop)) {
        ...
        if (state.isDraggingInternally) {
          if (draggedRange && !index_es$1.Range.equals(draggedRange, range) && !index_es$1.Editor.void(editor, {
            at: range,
            voids: true
          })) {
            index_es$1.Transforms.delete(editor, {
    ...
    onFocus: React.useCallback(event => {
      if (!readOnly && !state.isUpdatingSelection && ReactEditor.hasEditableTarget(editor, event.target) && !isEventHandled(event, attributes.onFocus)) {
        ...
        if (index_es.IS_FIREFOX && event.target !== el) {
          el.focus();
          return;
        }
        index_es.IS_FOCUSED.set(editor, true);
    ...
    onKeyDown: React.useCallback(event => {
      if (!readOnly && ReactEditor.hasEditableTarget(editor, event.target)) {
        ...
        var isRTL = index.default(index_es$1.Node.string(element)) === 'rtl';
        // COMPAT: Since we prevent the default behavior on
        // `beforeinput` events, the browser doesn't think there's ever
        // any history stack to undo or redo, so we have to manage these
        // hotkeys ourselves. (2019/11/06)
        if (index_es.Hotkeys.isRedo(nativeEvent)) {
          event.preventDefault();
        ...
        if (index_es.Hotkeys.isUndo(nativeEvent)) {
          event.preventDefault();
        ...
        // (2017/10/17)
        if (index_es.Hotkeys.isMoveLineBackward(nativeEvent)) {
          event.preventDefault();
          index_es$1.Transforms.move(editor, {
            unit: 'line',
            reverse: true
          });
          return;
        }
        if (index_es.Hotkeys.isMoveLineForward(nativeEvent)) {
          event.preventDefault();
          index_es$1.Transforms.move(editor, {
            unit: 'line'
          });
        ...
        if (index_es.Hotkeys.isExtendLineBackward(nativeEvent)) {
          event.preventDefault();
          index_es$1.Transforms.move(editor, {
            unit: 'line',
            edge: 'focus',
        ...
        if (index_es.Hotkeys.isExtendLineForward(nativeEvent)) {
          event.preventDefault();
          index_es$1.Transforms.move(editor, {
            unit: 'line',
            edge: 'focus'
        ...
        // string.
        if (index_es.Hotkeys.isMoveBackward(nativeEvent)) {
          event.preventDefault();
          if (selection && index_es$1.Range.isCollapsed(selection)) {
            index_es$1.Transforms.move(editor, {
              reverse: !isRTL
            });
          } else {
            index_es$1.Transforms.collapse(editor, {
              edge: isRTL ? 'end' : 'start'
            });
          }
          return;
        }
        if (index_es.Hotkeys.isMoveForward(nativeEvent)) {
          event.preventDefault();
          if (selection && index_es$1.Range.isCollapsed(selection)) {
            index_es$1.Transforms.move(editor, {
              reverse: isRTL
        ...
        if (index_es.Hotkeys.isMoveWordBackward(nativeEvent)) {
          event.preventDefault();
          if (selection && index_es$1.Range.isExpanded(selection)) {
            index_es$1.Transforms.collapse(editor, {
              edge: 'focus'
        ...
        if (index_es.Hotkeys.isMoveWordForward(nativeEvent)) {
          event.preventDefault();
          if (selection && index_es$1.Range.isExpanded(selection)) {
            index_es$1.Transforms.collapse(editor, {
              edge: 'focus'
        ...
        if (!index_es.HAS_BEFORE_INPUT_SUPPORT) {
          ...
          if (index_es.Hotkeys.isBold(nativeEvent) || index_es.Hotkeys.isItalic(nativeEvent) || index_es.Hotkeys.isTransposeCharacter(nativeEvent)) {
            event.preventDefault();
            return;
          }
          if (index_es.Hotkeys.isSoftBreak(nativeEvent)) {
            event.preventDefault();
            index_es$1.Editor.insertSoftBreak(editor);
            return;
          }
          if (index_es.Hotkeys.isSplitBlock(nativeEvent)) {
            event.preventDefault();
            index_es$1.Editor.insertBreak(editor);
            return;
          }
          if (index_es.Hotkeys.isDeleteBackward(nativeEvent)) {
            event.preventDefault();
            if (selection && index_es$1.Range.isExpanded(selection)) {
              index_es$1.Editor.deleteFragment(editor, {
                direction: 'backward'
          ...
          if (index_es.Hotkeys.isDeleteForward(nativeEvent)) {
            event.preventDefault();
            if (selection && index_es$1.Range.isExpanded(selection)) {
              index_es$1.Editor.deleteFragment(editor, {
                direction: 'forward'
          ...
          if (index_es.Hotkeys.isDeleteLineBackward(nativeEvent)) {
            event.preventDefault();
            if (selection && index_es$1.Range.isExpanded(selection)) {
              index_es$1.Editor.deleteFragment(editor, {
                direction: 'backward'
          ...
          if (index_es.Hotkeys.isDeleteLineForward(nativeEvent)) {
            event.preventDefault();
            if (selection && index_es$1.Range.isExpanded(selection)) {
              index_es$1.Editor.deleteFragment(editor, {
                direction: 'forward'
          ...
          if (index_es.Hotkeys.isDeleteWordBackward(nativeEvent)) {
            event.preventDefault();
            if (selection && index_es$1.Range.isExpanded(selection)) {
              index_es$1.Editor.deleteFragment(editor, {
                direction: 'backward'
          ...
          if (index_es.Hotkeys.isDeleteWordForward(nativeEvent)) {
            event.preventDefault();
            if (selection && index_es$1.Range.isExpanded(selection)) {
              index_es$1.Editor.deleteFragment(editor, {
                direction: 'forward'
          ...
          if (index_es.IS_CHROME || index_es.IS_WEBKIT) {
            // COMPAT: Chrome and Safari support `beforeinput` event but do not fire
            // an event when deleting backwards in a selected void inline node
            if (selection && (index_es.Hotkeys.isDeleteBackward(nativeEvent) || index_es.Hotkeys.isDeleteForward(nativeEvent)) && index_es$1.Range.isCollapsed(selection)) {
              var currentNode = index_es$1.Node.parent(editor, selection.anchor.path);
              if (index_es$1.Node.isElement(currentNode) && index_es$1.Editor.isVoid(editor, currentNode) && (index_es$1.Editor.isInline(editor, currentNode) || index_es$1.Editor.isBlock(editor, currentNode))) {
                event.preventDefault();
                index_es$1.Editor.deleteBackward(editor, {
                  unit: 'block'
                });
    ...
    onPaste: React.useCallback(event => {
      if (!readOnly && ReactEditor.hasEditableTarget(editor, event.target) && !isEventHandled(event, attributes.onPaste)) {
        ...
        if (!index_es.HAS_BEFORE_INPUT_SUPPORT || index_es.isPlainTextOnlyPaste(event.nativeEvent) || index_es.IS_WEBKIT) {
          event.preventDefault();
...
var DefaultPlaceholder = _ref2 => {
  ...
  return (
    ...
    React.createElement("span", _objectSpread({}, attributes), children, index_es.IS_ANDROID && /*#__PURE__*/React.createElement("br", null))
  );
};
/**
...
var defaultScrollSelectionIntoView = (editor, domRange) => {
  ...
  var isBackward = !!editor.selection && index_es$1.Range.isBackward(editor.selection);
...
var isDOMEventTargetInput = event => {
  return index_es.isDOMNode(event.target) && (event.target instanceof HTMLInputElement || event.target instanceof HTMLTextAreaElement);
};
/**
...
var Slate = props => {
  ...
  React.useState(() => {
    if (!index_es$1.Node.isNodeList(initialValue)) {
      throw new Error("[Slate] initialValue is invalid! Expected a list of elements but got: ".concat(index_es$1.Scrubber.stringify(initialValue)));
    }
    if (!index_es$1.Editor.isEditor(editor)) {
      throw new Error("[Slate] editor is invalid! You passed: ".concat(index_es$1.Scrubber.stringify(editor)));
    }
  ...
  React.useEffect(() => {
    index_es.EDITOR_TO_ON_CHANGE.set(editor, onContextChange);
    return () => {
      index_es.EDITOR_TO_ON_CHANGE.set(editor, () => {});
    };
...
var withReact = function withReact(editor) {
  ...
  var e = editor;
  e = index_es.withDOM(e, clipboardFormatKey);
  var {
    onChange,
    apply,
    insertText
  ...
  if (index_es.IS_ANDROID) {
    e.insertText = (text, options) => {
      ...
      index_es.EDITOR_TO_PENDING_SELECTION.delete(e);
  ...
  e.onChange = options => {
    ...
    var maybeBatchUpdates = REACT_MAJOR_VERSION < 18 ? require$$1.unstable_batchedUpdates : callback => callback();
    maybeBatchUpdates(() => {
      onChange(options);
    });
  };
  ...
  e.apply = operation => {
    if (operation.type === 'move_node') {
      var parent = index_es$1.Node.parent(e, operation.path);
      var chunking = !!e.getChunkSize(parent);
      if (chunking) {
        var node = index_es$1.Node.get(e, operation.path);
...
exports.NODE_TO_INDEX = index_es.NODE_TO_INDEX;
exports.NODE_TO_PARENT = index_es.NODE_TO_PARENT;
...
//# sourceMappingURL=index.es.cjs.map
