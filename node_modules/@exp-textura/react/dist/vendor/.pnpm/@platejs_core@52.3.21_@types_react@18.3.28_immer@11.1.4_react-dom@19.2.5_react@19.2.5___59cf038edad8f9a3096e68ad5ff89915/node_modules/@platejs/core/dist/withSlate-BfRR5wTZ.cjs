...
const index$2 = require('../../../../../@platejs_slate@52.3.21/node_modules/@platejs/slate/dist/index.cjs');
const index_browser = require('../../../../../nanoid@5.1.11/node_modules/nanoid/index.browser.cjs');
const index = require('../../../../../@udecode_utils@52.3.4/node_modules/@udecode/utils/dist/index.cjs');
const merge = require('../../../../../../../_virtual/merge.cjs');
const index$1 = require('../../../../../zustand-x@6.2.1_react-dom@19.2.5_react@19.2.5__react@19.2.5_scheduler@0.27.0_zustand@5._14c22be228d6642d328e08205e63ae2d/node_modules/zustand-x/dist/lib/index.cjs');
const mergeWith = require('../../../../../../../_virtual/mergeWith.cjs');
const defaults = require('../../../../../../../_virtual/defaults.cjs');
const kebabCase = require('../../../../../../../_virtual/kebabCase.cjs');
const pick = require('../../../../../../../_virtual/pick.cjs');
const castArray = require('../../../../../../../_virtual/castArray.cjs');
const index_es = require('../../../../../slate@0.124.0/node_modules/slate/dist/index.es.cjs');
const isEqual = require('../../../../../../../_virtual/isEqual.cjs');
const isUndefined = require('../../../../../../../_virtual/isUndefined.cjs');
const omitBy = require('../../../../../../../_virtual/omitBy.cjs');
const index_es$2 = require('../../../../../slate-hyperscript@0.100.0_slate@0.124.0/node_modules/slate-hyperscript/dist/index.es.cjs');
const cloneDeep = require('../../../../../../../_virtual/cloneDeep.cjs');
const index_es$1 = require('../../../../../slate-dom@0.124.0_slate@0.124.0/node_modules/slate-dom/dist/index.es.cjs');
...
function mergePlugins(basePlugin, ...sourcePlugins) {
	return mergeWith.default({}, basePlugin, ...sourcePlugins, (objValue, srcValue, key) => {
...
function createSlatePlugin(config = {}) {
	...
	if (plugin.node.isLeaf && !index.isDefined(plugin.node.isDecoration)) plugin.node.isDecoration = true;
...
const resolvePlugin = (editor, _plugin) => {
	...
	if (targetPluginToInject && targetPlugins && targetPlugins.length > 0) {
		...
		plugin.inject.plugins = merge.default({}, plugin.inject.plugins, Object.fromEntries(targetPlugins.map((targetPlugin) => {
...
const resolvePlugins = (editor, plugins = [], createStore = index$1.createVanillaStore) => {
	editor.plugins = {};
	...
	editor.meta.pluginList.forEach((plugin) => {
		if (plugin.extendEditor) {
			...
			index$2.syncLegacyMethods(editor);
		}
		...
		plugin.__apiExtensions.forEach(({ extension, isOverride, isPluginSpecific, isTransform }) => {
			...
			if (isOverride) {
				if (newExtensions.api) {
					merge.default(editor.api, newExtensions.api);
					merge.default(plugin.api, newExtensions.api);
					index$2.assignLegacyApi(editor, editor.api);
				}
				if (newExtensions.transforms) {
					merge.default(editor.transforms, newExtensions.transforms);
					merge.default(plugin.transforms, newExtensions.transforms);
					index$2.assignLegacyTransforms(editor, newExtensions.transforms);
				}
				...
				merge.default(editor.transforms[plugin.key], newExtensions);
				merge.default(plugin.transforms[plugin.key], newExtensions);
			} else {
				merge.default(editor.transforms, newExtensions);
				merge.default(plugin.transforms, newExtensions);
				index$2.assignLegacyTransforms(editor, newExtensions);
			}
			else if (isPluginSpecific) {
				if (!editor.api[plugin.key]) editor.api[plugin.key] = {};
				if (!plugin.api[plugin.key]) plugin.api[plugin.key] = {};
				merge.default(editor.api[plugin.key], newExtensions);
				merge.default(plugin.api[plugin.key], newExtensions);
			} else {
				merge.default(editor.api, newExtensions);
				merge.default(plugin.api, newExtensions);
				index$2.assignLegacyApi(editor, editor.api);
			}
		});
...
const resolvePluginOverrides = (editor) => {
	const applyOverrides = (plugins) => {
		...
		overriddenPlugins = overriddenPlugins.map((p) => {
			...
			if (index.isDefined(enabled)) updatedPlugin.enabled = enabled;
...
const withPlateHistory = ({ editor }) => index$2.withHistory(editor);
		...
		if (editor.selection && editor.api.isCollapsed()) {
			...
			if (block) {
				...
				if (checkMatchRulesOverride("break.splitReset", blockNode, blockPath)?.splitReset ?? breakRules?.splitReset) {
					...
					insertBreak();
					editor.tf.resetBlock({ at: isAtStart ? blockPath : index$2.PathApi.next(blockPath) });
					return;
				}
			}
		}
	...
	return { transforms: {
		deleteBackward(unit) {
			if (editor.selection && editor.api.isCollapsed()) {
				...
				if (index$2.PointApi.equals(editor.selection.anchor, editor.api.start([]))) {
		...
		deleteFragment(options) {
			if (editor.selection && index$2.RangeApi.equals(editor.selection, editor.api.range([]))) {
				editor.tf.reset({
					children: true,
					select: true
				});
...
const withMergeRules = (ctx) => {
	...
	return {
		api: { shouldMergeNodes(prevNodeEntry, nextNodeEntry, { reverse } = {}) {
			...
			if (index$2.TextApi.isText(prevNode) && prevNode.text === "" && prevPath.at(-1) !== 0) {
			...
			if (index$2.ElementApi.isElement(targetNode) && editor.api.isVoid(targetNode)) {
				if (shouldRemove(targetNode, targetPath)) editor.tf.removeNodes({ at: prevPath });
				else if (index$2.ElementApi.isElement(curNode) && editor.api.isEmpty(curNode)) editor.tf.removeNodes({ at: curPath });
				return false;
			}
			if (index$2.ElementApi.isElement(prevNode) && editor.api.isEmpty(prevNode) && index$2.PathApi.isSibling(prevPath, nextPath) && shouldRemove(prevNode, prevPath)) {
				editor.tf.removeNodes({ at: prevPath });
				return false;
			}
			return true;
		...
		transforms: { removeNodes(options = {}) {
			if (options.event?.type === "mergeNodes" && options.at) {
				...
				if (nodeEntry) {
					...
					if (index$2.ElementApi.isElement(node)) {
						const plugin = getPluginByType(editor, node.type);
						if (plugin) {
...
const withNormalizeRules = (ctx) => {
	...
	return { transforms: { normalizeNode([node, path]) {
		if (index$2.ElementApi.isElement(node) && node.type) {
...
const applyDeepToNodes = ({ apply, node, path = [], query, source }) => {
	if (index$2.queryNode([node, path], query)) if (typeof source === "function") apply(node, source());
	else apply(node, source);
	if (!index$2.NodeApi.isAncestor(node)) return;
...
const getInjectMatch = (editor, plugin) => {
	return (node, path) => {
		...
		const element = index$2.ElementApi.isElement(node) ? node : void 0;
		...
		if (excludeBelowPlugins || maxLevel) {
			...
			if (excludeBelowPlugins) {
				...
				if (editor.api.above({
					at: path,
					match: (n) => index$2.ElementApi.isElement(n) && excludeTypes.includes(n.type)
				})) return false;
			}
		}
		return true;
...
const getNodeDataAttributeKeys = (node) => Object.keys(node).filter((key) => typeof node[key] !== "object" && (!index$2.TextApi.isText(node) || key !== "text")).map((key) => keyToDataAttribute(key));
const keyToDataAttribute = (key) => `data-slate-${kebabCase.default(key)}`;
...
const getEdgeNodes = (editor) => {
	...
	if (!edge) return null;
	const parent = index$2.NodeApi.parent(editor, cursor.path) ?? null;
	/** Inline elements */
	const isAffinityInlineElement = (() => {
		if (!parent || !index$2.ElementApi.isElement(parent)) return false;
	...
	const nodeEntry = isAffinityInlineElement ? [parent, index$2.PathApi.parent(cursor.path)] : [index$2.NodeApi.get(editor, cursor.path), cursor.path];
	if (edge === "start" && cursor.path.at(-1) === 0 && !isAffinityInlineElement) return [null, nodeEntry];
	const siblingPath = edge === "end" ? index_es.Path.next(nodeEntry[1]) : index_es.Path.previous(nodeEntry[1]);
	const siblingNode = index$2.NodeApi.get(editor, siblingPath);
...
const getMarkBoundaryAffinity = (editor, markBoundary) => {
	...
	if (!selection) return;
	const marksMatchLeaf = (leaf) => marks && isEqual.default(index$2.NodeApi.extractProps(leaf), marks) && Object.keys(marks).length > 1;
	...
	if (index_es$1.IS_FIREFOX && selectionDirection === "forward" && marksDirection !== "backward") return "forward";
	return "backward";
};
...
const isNodeAffinity = (editor, node, affinity) => {
	const marks = Object.keys(index$2.NodeApi.extractProps(node));
	return (index$2.ElementApi.isElement(node) ? [node.type] : marks).some((type) => getPluginByType(editor, type)?.rules.selection?.affinity === affinity);
};
...
const setAffinitySelection = (editor, edgeNodes, affinity) => {
	...
	if (affinity === "backward") {
		...
		if (index$2.ElementApi.isElement(before[0])) return;
		setMarks(null);
		return;
	}
	if (before === null) {
	...
	if (index$2.ElementApi.isElement(after[0])) return;
	setMarks(index$2.NodeApi.extractProps(after[0]));
};
...
const AffinityPlugin = createTSlatePlugin({ key: "affinity" }).overrideEditor(({ editor, tf: { deleteBackward, insertText, move } }) => ({ transforms: {
	deleteBackward: (unit) => {
		const apply = () => {
			if (unit === "character" && editor.api.isCollapsed()) {
				...
				const startText = start && (index$2.TextApi.isText(start[0]) ? start[0].text : index$2.NodeApi.string(start[0]));
				deleteBackward(unit);
	...
	insertText(text, options) {
		...
		const applyOutwardAffinity = () => {
			...
			const textNode = index$2.NodeApi.get(editor, textPath);
			if (!textNode) return;
			const outwardMarks = Object.keys(index$2.NodeApi.extractProps(textNode)).filter((type) => getPluginByType(editor, type)?.rules.selection?.affinity === "outward");
			...
			if (nextPoint) {
				...
				nextTextNode = index$2.NodeApi.get(editor, nextTextPath) || null;
			}
...
const hasElement = (edgeNodes) => {
	const [before, after] = edgeNodes;
	return before && index$2.ElementApi.isElement(before[0]) || after && index$2.ElementApi.isElement(after[0]);
};
...
const ChunkingPlugin = createTSlatePlugin({
	...
	options: {
		...
		query: index$2.NodeApi.isEditor
	}
...
const withScrolling = (editor, fn, options) => {
	...
	if (options) {
		const scrollOptions = typeof options.scrollOptions === "object" && options.scrollOptions ? {
			...
			...omitBy.default(options.scrollOptions, isUndefined.default)
		} : options.scrollOptions ?? prevOptions.scrollOptions;
		const ops = {
			...prevOptions,
			scrollOperations: {
				...prevOptions.scrollOperations,
				...omitBy.default(options.operations ?? {}, isUndefined.default)
			},
			scrollOptions,
			...omitBy.default({ scrollMode: options.mode }, isUndefined.default)
		};
...
}).extendEditorApi(({ editor }) => ({ isScrolling: () => AUTO_SCROLL.get(editor) ?? false })).extendEditorTransforms(({ editor }) => ({ withScrolling: index.bindFirst(withScrolling, editor) })).overrideEditor(({ api, editor, getOption, tf: { apply } }) => ({ transforms: { apply(operation) {
	if (api.isScrolling()) {
		apply(operation);
...
const htmlBodyToFragment = (editor, element) => {
	if (element.nodeName === "BODY") return index_es$2.jsx("fragment", {}, deserializeHtmlNodeChildren(editor, element));
};
...
const pluginDeserializeHtml = (editor, plugin, { deserializeLeaf, element: el }) => {
	...
	if (rules) {
		if (!rules.some(({ validAttribute, validClassName, validNodeName = "*", validStyle }) => {
			if (validNodeName) {
				const validNodeNames = castArray.default(validNodeName);
			...
			if (validStyle) for (const [key, value] of Object.entries(validStyle)) {
				if (!castArray.default(value).includes(el.style[key]) && value !== "*") return false;
			...
			if (validAttribute) if (typeof validAttribute === "string") {
				...
				const attributeValues = castArray.default(attributeValue);
				const elAttribute = el.getAttribute(attributeName);
				if (!index.isDefined(elAttribute) || !attributeValues.includes(elAttribute)) return false;
			}
			return true;
		})) return;
	}
...
const htmlElementToElement = (editor, element, isSlate = false) => {
	...
	if (deserialized) {
		...
		return index_es$2.jsx("element", node, descendants);
	}
};
...
const htmlElementToLeaf = (editor, element) => {
	...
	return deserializeHtmlNodeChildren(editor, element).reduce((arr, child) => {
		if (!child) return arr;
		if (index$2.ElementApi.isElement(child)) {
			if (Object.keys(node).length > 0) mergeDeepToNodes({
				node: child,
				query: { filter: ([n]) => index$2.TextApi.isText(n) },
				source: node
			});
			arr.push(child);
		} else {
			const attributes = { ...node };
			if (index$2.TextApi.isText(child) && child.text) Object.keys(attributes).forEach((key) => {
				if (attributes[key] && child[key]) attributes[key] = child[key];
			});
			arr.push(index_es$2.jsx("text", attributes, child));
		}
		return arr;
	}, []);
};
...
*/
const HtmlPlugin = createSlatePlugin({ key: "html" }).extendApi(({ editor }) => ({ deserialize: index.bindFirst(deserializeHtml, editor) })).extend({ parser: {
	format: "text/html",
	...
	return { transforms: {
		apply(operation) {
			...
			if (operation.type === "insert_node") {
				const node = cloneDeep.default(operation.node);
				applyDeepToNodes({
			...
			if (operation.type === "split_node") {
				...
				if (index$2.queryNode([node, operation.path], query)) {
					/**
					* Create a new id if:
					*
		...
		insertNode(node) {
			...
			if (!disableInsertOverrides && node[idKey]) {
				if (!Object.isExtensible(node)) node = cloneDeep.default(node);
				node._id = node[idKey];
			}
			insertNode(node);
		},
		insertNodes(_nodes, options) {
			const nodes = castArray.default(_nodes).filter((node) => !!node);
...
const NodeIdPlugin = createTSlatePlugin({
	...
	options: {
		...
		filter: () => true,
		idCreator: () => index_browser.nanoid(10)
	}
	...
	const addNodeId = (entry) => {
		...
		if (!node[idKey] && index$2.queryNode([node, path], {
			allow,
			exclude,
			filter: (entry$1) => {
				const [node$1] = entry$1;
				if (filterText && !index$2.ElementApi.isElement(node$1)) return false;
				if (filterInline && index$2.ElementApi.isElement(node$1) && !editor.api.isBlock(node$1)) return false;
		...
		if (index$2.ElementApi.isElement(node)) node.children.forEach((child, index) => {
...
const insertExitBreak = (editor, { match, reverse } = {}) => {
	...
	const ancestorPath = editor.api.above({
		at: block[1],
		match: index$2.combineMatchOptions(editor, (n, p) => p.length === 1 || p.length > 1 && !!n.type && !getPluginByType(editor, n.type)?.node.isStrictSiblings, { match })
	})?.[1] ?? block[1];
	const targetPath = reverse ? ancestorPath : index$2.PathApi.next(ancestorPath);
...
const resetBlock = (editor, { at } = {}) => {
	...
	editor.tf.withoutNormalizing(() => {
		const { type, ...otherProps } = index$2.NodeApi.extractProps(block);
...
const SlateExtensionPlugin = createTSlatePlugin({
	...
	return {
		init: index.bindFirst(init, editor),
		insertExitBreak: index.bindFirst(insertExitBreak, editor),
		resetBlock: index.bindFirst(resetBlock, editor),
		setValue: index.bindFirst(setValue, editor),
		apply(operation) {
			const noop = () => {};
			...
			let text;
			let parentNode;
			if (index$2.OperationApi.isNodeOperation(operation) && hasNodeHandlers) switch (operation.type) {
				case "insert_node":
				...
				case "set_node":
				case "split_node":
					prevNode = index$2.NodeApi.get(editor, operation.path);
					break;
				case "remove_node":
					prevNode = operation.node;
					node = operation.node;
			...
			else if (index$2.OperationApi.isTextOperation(operation) && hasTextHandlers) {
				const parentPath = index$2.PathApi.parent(operation.path);
				parentNode = index$2.NodeApi.get(editor, parentPath);
				prevText = index$2.NodeApi.get(editor, operation.path).text;
			}
			apply(operation);
			if (index$2.OperationApi.isNodeOperation(operation) && hasNodeHandlers) {
				switch (operation.type) {
					case "insert_node":
					case "remove_node": break;
					case "merge_node": {
						const prevPath = index$2.PathApi.previous(operation.path);
						if (prevPath) node = index$2.NodeApi.get(editor, prevPath);
						break;
					}
					case "move_node":
						node = index$2.NodeApi.get(editor, operation.newPath);
						break;
					case "set_node":
						node = index$2.NodeApi.get(editor, operation.path);
						break;
					case "split_node":
						node = index$2.NodeApi.get(editor, operation.path);
						break;
			...
			if (index$2.OperationApi.isTextOperation(operation) && hasTextHandlers) {
				const textNodeAfter = index$2.NodeApi.get(editor, operation.path);
...
const isInlineNode = (editor) => (node) => index$2.TextApi.isText(node) || index$2.ElementApi.isElement(node) && editor.api.isInline(node);
...
const normalize = (descendants, isInline, makeDefaultBlock) => {
	...
	descendants = descendants.map((node) => {
		if (index$2.ElementApi.isElement(node)) return {
			...node,
...
const withSlate = (e, { id, affinity = true, autoSelect, chunking = true, maxLength, nodeId, optionsStoreFactory, plugins = [], readOnly = false, rootPlugin, selection, shouldNormalizeEditor, skipInitialization, userId, value, onReady, ...pluginConfig } = {}) => {
	const editor = e;
	editor.id = id ?? editor.id ?? index_browser.nanoid();
	editor.meta.key = editor.meta.key ?? index_browser.nanoid();
...
exports.$ = DebugPlugin;
exports.A = pluginDeserializeHtml;
exports.At = withNormalizeRules;
exports.B = collapseWhiteSpaceText;
exports.Bt = getPluginKey;
exports.C = deserializeHtmlElement;
exports.Ct = isSlatePluginNode;
exports.D = pipeDeserializeHtmlLeaf;
exports.Dt = applyDeepToNodes;
exports.E = htmlElementToLeaf;
exports.Et = isSlateVoid;
exports.F = collapseWhiteSpace;
exports.Ft = HistoryPlugin;
exports.G = isHtmlBlockElement;
exports.Gt = getEditorPlugin;
exports.H = upsertInlineFormattingContext;
exports.Ht = getPluginType;
exports.I = collapseWhiteSpaceElement;
exports.It = withPlateHistory;
exports.J = isHtmlText;
exports.K = isHtmlInlineElement;
exports.Kt = createSlatePlugin;
exports.L = inferWhiteSpaceRule;
exports.Lt = AstPlugin;
exports.M = htmlBrToNewLine;
exports.Mt = withDeleteRules;
exports.N = htmlBodyToFragment;
exports.Nt = withBreakRules;
exports.O = htmlElementToElement;
exports.Ot = OverridePlugin;
exports.P = deserializeHtmlNodeChildren;
exports.Pt = BaseParagraphPlugin;
exports.Q = withScrolling;
exports.R = collapseWhiteSpaceChildren;
exports.S = htmlStringToDOMNode;
exports.T = htmlTextNodeToString;
exports.Tt = isSlateText;
exports.U = isLastNonEmptyTextOfInlineFormattingContext;
exports.V = endInlineFormattingContext;
exports.Vt = getPluginKeys;
exports.W = collapseString;
exports.Wt = getSlatePlugin;
exports.X = AUTO_SCROLL;
exports.Y = isHtmlElement;
exports.Z = DOMPlugin;
exports._ = withNodeId;
exports.a = pipeInsertDataQuery;
exports.at = isNodeAffinity;
exports.b = parseHtmlDocument;
exports.bt = isSlateLeaf;
exports.c = setValue;
exports.ct = getEdgeNodes;
exports.d = init;
exports.dt = getPluginNodeProps;
exports.et = PlateError;
exports.f = isEditOnly;
exports.ft = getNodeDataAttributeKeys;
exports.gt = defaultsDeepToNodes;
exports.h = NodeIdPlugin;
exports.ht = getInjectMatch;
exports.i = ParserPlugin;
exports.it = setAffinitySelection;
exports.j = getDataNodeProps;
exports.jt = withMergeRules;
exports.k = pipeDeserializeHtmlElement;
exports.kt = withOverrides;
exports.l = resetBlock;
exports.lt = mergeDeepToNodes;
exports.m = pipeOnNodeChange;
exports.mt = getInjectedPlugins;
exports.n = withSlate;
exports.nt = withChunking;
exports.o = normalizeDescendantsToDocumentFragment;
exports.ot = isNodesAffinity;
exports.p = pipeOnTextChange;
exports.pt = keyToDataAttribute;
exports.q = inlineTagNames;
exports.qt = createTSlatePlugin;
exports.r = getCorePlugins;
exports.rt = AffinityPlugin;
exports.s = SlateExtensionPlugin;
exports.st = getMarkBoundaryAffinity;
exports.tt = ChunkingPlugin;
exports.u = insertExitBreak;
exports.ut = getSlateClass;
exports.v = LengthPlugin;
exports.w = deserializeHtmlNode;
exports.wt = isSlateString;
exports.x = deserializeHtml;
exports.xt = isSlateNode;
exports.y = HtmlPlugin;
exports.yt = isSlateElement;
exports.z = collapseWhiteSpaceNode;
exports.zt = getPluginByType;
//# sourceMappingURL=withSlate-BfRR5wTZ.cjs.map
