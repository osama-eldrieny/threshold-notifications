...
const withSlateBfRR5wTZ = require('./withSlate-BfRR5wTZ.cjs');
const index$1 = require('../../../../../@platejs_slate@52.3.21/node_modules/@platejs/slate/dist/index.cjs');
...
const pluginInjectNodeProps = (editor, plugin, nodeProps, getElementPath) => {
	...
	if (!withSlateBfRR5wTZ.ht(editor, plugin)(node, getElementPath(node))) return;
	const queryResult = query?.({
		...injectNodeProps,
		...withSlateBfRR5wTZ.Gt(editor, plugin),
		nodeProps
	});
	if (query && !queryResult) return;
	const nodeValue = node[nodeKey];
	if (!transformProps && (!index.isDefined(nodeValue) || validNodeValues && !validNodeValues.includes(nodeValue) || nodeValue === defaultNodeValue)) return;
...
const pipeInjectNodeProps = (editor, nodeProps, getElementPath, readOnly = false) => {
	editor.meta.pluginCache.inject.nodeProps.forEach((key) => {
		...
		if (withSlateBfRR5wTZ.f(readOnly, plugin, "inject")) return;
...
const getSelectedDomFragment = (editor) => {
	...
	domBlocks.forEach((node, index) => {
		...
		*/
		if ((index === 0 || index === domBlocks.length - 1) && node.textContent?.trim() !== index$1.NodeApi.string(block[0]) && index$1.ElementApi.isElement(block[0]) && !editor.api.isVoid(block[0])) {
...
withSlateBfRR5wTZ.Z.extendEditorApi(({ editor }) => ({ getFragment() {
...
const useNodeAttributes = (props, ref) => ({
	...props.attributes,
	className: clsx$1.clsx(props.attributes.className, props.className) || void 0,
	ref,
	style: {
...
const getNodeDataAttributes = (editor, node, { isElement, isLeaf, isText }) => {
	return Object.keys(node).reduce((acc, key) => {
		...
		const attributeName = withSlateBfRR5wTZ.pt(key);
...
const getPluginDataAttributes = (editor, plugin, node) => {
	...
	const customAttributes = plugin.node.toDataAttributes?.({
		...plugin ? withSlateBfRR5wTZ.Gt(editor, plugin) : {},
		node
	}) ?? {};
	return {
		...dataAttributes,
...
const getRenderNodeStaticProps = ({ attributes: nodeAttributes, editor, node, path, plugin, props }) => {
	...
	const pluginProps = withSlateBfRR5wTZ.dt({
	...
	newProps = {
		...
		attributes: {
			...
			className: clsx$1(withSlateBfRR5wTZ.ut(plugin?.node.type), className) || void 0
		}
	};
...
const pipeRenderElementStatic = (editor, { renderElement: renderElementProp } = {}) => function render(props) {
	const plugin = withSlateBfRR5wTZ.zt(editor, props.element.type);
...
const ElementStatic = React.memo(BaseElementStatic, (prev, next) => (prev.element === next.element || prev.element._memo !== void 0 && prev.element._memo === next.element._memo) && index_es.isElementDecorationsEqual(prev.decorations, next.decorations));
function BaseLeafStatic({ decorations, editor, path, text = { text: "" } }) {
	...
	const leafElements = index$1.TextApi.decorations(text, decorations).map(({ leaf, position }, index) => {
...
const LeafStatic = React.memo(BaseLeafStatic, (prev, next) => {
	return index$1.TextApi.equals(next.text, prev.text) && index_es.isTextDecorationsEqual(next.decorations, prev.decorations);
});
	...
	return /* @__PURE__ */ React.createElement(React.Fragment, null, children.map((child, i) => {
		...
		if (range) {
			...
			for (const dec of decorations) {
				const d = index$1.RangeApi.intersection(dec, range);
				if (d) ds.push(d);
			}
		}
		return index$1.ElementApi.isElement(child) ? /* @__PURE__ */ React.createElement(ElementStatic, {
			key: i,
			decorate,
			decorations: ds,
			editor,
...
exports.A = pipeInjectNodeProps;
exports.D = getSelectedDomNode;
exports.E = isSelectOutside;
exports.O = getSelectedDomFragment;
exports.S = useNodeAttributes;
exports._ = getPluginDataAttributes;
exports.a = ElementStatic;
exports.b = SlateLeaf;
exports.c = pipeRenderTextStatic;
exports.d = pluginRenderElementStatic;
exports.g = getNodeDataAttributes;
exports.h = getRenderNodeStaticProps;
exports.i = pluginRenderLeafStatic;
exports.k = pipeDecorate;
exports.l = pluginRenderTextStatic;
exports.o = LeafStatic;
exports.r = pipeRenderLeafStatic;
exports.u = pipeRenderElementStatic;
exports.x = SlateText;
exports.y = SlateElement;
//# sourceMappingURL=static-DIYyt_jS.cjs.map
